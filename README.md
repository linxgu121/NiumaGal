...
- `DialogueRoot`：挂 `NiumaUIDialogueViewBridge`，绑定 UIManager、DialoguePresenter、NiumaDialogueController。第三人称项目建议把 Cursor Mode 保持为 `VisibleWhenChoices`，这样只有出现选项时才解锁并显示鼠标；同时绑定 PlayerCameraManager 或开启自动查找，用于对话期间隐藏 TPC 准心。
- `DialogueRoot` 或 `UIRoot/AmbientNarrative`：旧 UGUI 场景挂 `AmbientNarrativeUIViewBridge`；UI Toolkit 场景优先挂 `NiumaGalToolkitAmbientNarrativeBridge`，绑定 DialoguePresenter、UIToolkitUIManager 和 WorldCamera。
...
`NiumaUIDialogueViewBridge` 的 Cursor Mode 有三种：
...
`NiumaUIDialogueViewBridge` 提供两类隐藏入口：

- `Hide TPC Crosshair During Dialogue`：默认开启。会在对话开始时把 `PlayerCameraManager.ShowCrosshair` 临时设为 `false`，对话结束后恢复进入对话前的值。
- `Hide Objects During Dialogue`：拖入需要临时隐藏的 GameObject，例如交互提示根物体、普通准心 UI、小地图、任务追踪面板、Gameplay HUD 根节点等。桥接层会记录每个物体进入对话前的 `activeSelf`，结束时按原状态恢复。

推荐绑定方式：

1. `DialogueRoot/NiumaUIDialogueViewBridge` 上开启 `Hide TPC Crosshair During Dialogue`。
2. 如果场景中只有一个 `PlayerCameraManager`，可以保持 `Auto Find Player Camera Manager = true`。
3. 如果场景中可能有多个相机管理器，手动把当前玩家的 `PlayerCameraManager` 拖到 `Player Camera Manager`。
4. 把 `UIRoot/GameplayHUD`、`UIRoot/InteractionPrompt`、`UIRoot/Reticle` 等普通 Gameplay UI 拖进 `Hide Objects During Dialogue`。
5. 不要把 `UIRoot/DialogueWindow` 或它的父物体拖进隐藏列表，否则会把对话框自己隐藏掉。
...
用途：旧 UGUI 对话窗口桥接。项目迁移到 UI Toolkit 后，新场景优先使用 `NiumaGalToolkitDialogueViewBridge`，本脚本只作为 Legacy 过渡。
...
用途：UI Toolkit 对话窗口桥接。它把 `DialoguePresenter` 和 `DialogueService` 的当前句子、打字机文本、选项数据转换为 NiumaUI 的 `DialogueToolkitViewData`，再推给 `DialogueToolkitBindingProvider`。
...
用途：旧 UGUI 对话窗口。新 UI Toolkit 场景不需要挂它。
